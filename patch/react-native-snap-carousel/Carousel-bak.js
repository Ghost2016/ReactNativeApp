...
export default class Carousel extends Component {
    ...
    _onScroll (event) {
        const { callbackOffsetMargin, enableMomentum, onScroll, isIosScroll } = this.props;
        ...
        if (enableMomentum) {
            ...
            if (itemReached) {
                if (this._canFireBeforeCallback) {
                    const isReturn = this._onBeforeSnap(this._getDataIndex(nextActiveItem));
                    if(isReturn) {
                        return;
                    }
                }
    ...
    _onScrollBeginDrag (event) {
        ...
        if(this._nextDragTime >= Date.now()) {
            this._lockScroll();
    ...
    _onBeforeSnap (index) {
        ...
        this._canFireBeforeCallback = false;
        if(onBeforeSnapToItem)  {
            return onBeforeSnapToItem(index);
        } else {
            return undefined;
        }
    }
