...
const { defaultHeight, width, pie, legend, pieColorScaleBlue, pieColorScaleMix } = props.pie;
 ...
 * 面积图
 */
export default class SlicedPieChart extends Component {
  constructor() {
    super();
    this.state = {};
  }
  ...
  render() {
    ...
    const { data, height, type, width } = this.props;
    const pieColors = type === 'big' ? pieColorScaleMix : pieColorScaleBlue
    let category = [];
    let formattedData = [];
    // 整理数据
    var total = 0;
    ...
    // const nodata = this.props.nodata || (data.length === 0)
    // console.warn('ly88', 'nodata', nodata)
    ...
    // 饼图padding
    // const piePadding = type==='small' ? this.getPixel(60):this.getPixel(20)
    ...
    return (
      ...
      {nodata && (<CircleChart
        ...
        {!nodata && <VictoryPie
            ...
            data={formattedData}
            width={pieWidth}
            // width={height + (type === "small" ? CSS.pixel(100) : 0)}
            height={height}
            innerRadius={type==='small' ? CSS.pixel(46) : CSS.pixel(70)}
            // innerRadius={type==='small' ? this.getPixel(60) : this.getPixel(30)}
            // labelRadius={this.getPixel(90)}
                    ...
                    return [
                      {
                        target: "data",
                        mutation: props => {
                          this.handleClick("press-pie-in", props);
                        }
                      }
                    ];
                  },
                    ...
                    return [
                      {
                        target: "data",
                        mutation: props => {
                          this.handleClick("press-pie-out", props);
                        }
                      }
                    ];
                  },
                    ...
                    return [
                      {
                        target: "data",
                        mutation: props => {
                          this.handleClick("press-pie", props);
                        }
                      }
                    ];
                  }
          ...
          <VictoryLegend
            ...
            // x={height + CSS.pixel(250 - height)}
            // 使legend居中
            // y={height / 2 - (CSS.pixel(38) * category.length) / 2}
            // y={(height - pieWidth)/2 + piePadding}
            y={0}
            data={
              category.map((item,index) => {
                return {...item,name:''}
                // return {...item,name:item.name +'    ' + formattedData[index].y + '%'}
              })
            }
