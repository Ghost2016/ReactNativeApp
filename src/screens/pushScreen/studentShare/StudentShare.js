...
import StudentShareTab from "./StudentShareTab";
...
class StudentShare extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      newsList: [],
  ...
  componentDidMount() {
    ...
    this.context.refs["_studentShare"] = this;
  }

  componentWillUnmount() {
    if (this.context.refs["_studentShare"]) {
      delete this.context.refs["_studentShare"];
    }
  }
    ...
    if (event.type == "NavBarButtonPress") {
      if (event.id == "filter_studentNewsBtn") {
        navLightBox("LightBoxScreen", {
          passProps: {
            screen: () => <FilterLightbox categoryId={this.state.categoryIds[this.state.startIndex]} />
          }
        });
      }
    }
  ...
  showDetail(detail, titlePos, closeEvent) {
    const duration = 400;
      ...
      >
        ...
        <SDTabs2
          ...
          tabWidthStyle={{
            ...
            borderColor: "#f00",
            backgroundColor: '#fff',
            paddingBottom: CSS.pixel(16, true),
          }}
          tabContentStyle={{
            height: '100%'
          }}
        >
          <StudentShareTab
            categoryId={this.state.categoryIds[0]}
          />
          <StudentShareTab
            categoryId={this.state.categoryIds[1]}
          />
          <StudentShareTab
            categoryId={this.state.categoryIds[2]}
          />
          <StudentShareTab
            categoryId={this.state.categoryIds[3]}
          />
        </SDTabs2>
        ...
        >
                              ...
                              .then(installed => {
                                if (installed) {
                                  WeChat.shareToSession({
                                    ...
                                    thumbImage: this.state.detail && this.state.detail.image ? this.state.detail.image.url : `${HOSTS.SHARE}/images/logo.png`,
          ...
          >
            ...
            <TabsButton
              ...
              categoryId={this.state.categoryIds[this.state.startIndex] == 0 ? "0" : this.state.categoryIds[this.state.startIndex] + ""}
            />
          </Animated.View>
        </View>
      </View>
